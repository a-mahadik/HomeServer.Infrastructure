...
On the self-hosted runner (or via the MinIO console at `http://localhost:9001`):

```sh
mc alias set local http://localhost:9000 "$MINIO_ROOT_USER" "$MINIO_ROOT_PASSWORD"
mc mb local/tf-state
```
...
3. **Terraform Init** receives `-backend-config` flags that point at `http://localhost:9000`, the `tf-state` bucket, and the MinIO credentials. Additional flags (`skip_credentials_validation`, `force_path_style`, etc.) are required because MinIO is not AWS S3.
4. Terraform reads and writes state directly in the MinIO bucket — no local file backend.
